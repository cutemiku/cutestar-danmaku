...
async function handleEnter() {
  ...
  try {
    ...
    state.customColor = sanitizeColor(activity.danmaku_default_color || '#FFFFFF');
...
function renderSender() {
        ...
        ${state.danmakuHistory.length === 0
          ...
          : state.danmakuHistory.map((d, i) => `
            <div class="danmaku-item" style="animation-delay: ${i * 0.05}s">
              ...
              <div class="danmaku-bubble" style="border-left-color: ${sanitizeColor(d.color)}">
                <div class="danmaku-nick">${escapeHtml(d.nickname)}</div>
                <div class="danmaku-text" style="color: ${sanitizeColor(d.color)}">${escapeHtml(d.content)}</div>
              </div>
            </div>
          `).join('')
        }
    ...
    <div class="sender-footer">
      ...
      <div class="input-toolbar">
        ${state.allowCustomColor ? `
          <button id="color-toggle" class="color-btn" style="background: ${sanitizeColor(state.customColor)}">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      ${state.showColorPicker ? `
        <div class="color-panel">
          ...
          <div class="color-custom">
            <input type="color" id="custom-color" value="${sanitizeColor(state.customColor)}">
            <span>${sanitizeColor(state.customColor)}</span>
          </div>
        </div>
      ` : ''}
  ...
  container.querySelectorAll('.color-swatch').forEach(btn => {
    btn.addEventListener('click', () => {
      state.customColor = sanitizeColor(btn.dataset.color);
      render();
    });
  });
  ...
  if (customColorInput) {
    customColorInput.addEventListener('input', (e) => {
      state.customColor = sanitizeColor(e.target.value);
      render();
    });
  }
...
// 颜色白名单校验：仅允许 #RRGGBB，非法回退白色（防 CSS style 属性注入）
